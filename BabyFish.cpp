...
ObjectPtr restart_button1, restart_button2;
ObjectPtr upper_bar;
	...
	if (rn < 20) {
		...
		move_t->setOnTimerCallback([&](TimerPtr t)->bool {
			if (IsContact(player, fish) == 1) {
				...
				gamescore += fish.get_score();
				UpdateScore(gamescore, score_card);
				FishCount(fish.get_fishnum());
				CheckMission(mission_n);
				return true;
			}
			if (IsContact(player, fish) == -1 && !player.is_damaged()) {
				damaged_s->play();
				if (player.is_eating()) {
					eat_t->stop();
					eat_t->set(2.0f);
				}
				player.change_damaged();
				damaged_t->start();
				life_count--;
				switch (life_count) {
				case 2:
...
void InitGame() {
	...
	gameplay->setOnKeyboardCallback([&](ScenePtr scene, KeyCode key, bool pressed)->bool {
		if (key == KeyCode::KEY_A || key == KeyCode::KEY_CAPITAL_A) {
			if (pressed) 
				leftT->start();
			else
				leftT->stop();
		...
		if (key == KeyCode::KEY_W || key == KeyCode::KEY_CAPITAL_W) {
			if (pressed)
				upT->start();
			else
				upT->stop();
		...
		if (key == KeyCode::KEY_D || key == KeyCode::KEY_CAPITAL_D) {
			if (pressed)
				rightT->start();
			else
				rightT->stop();
		...
		if (key == KeyCode::KEY_S || key == KeyCode::KEY_CAPITAL_S) {
			if (pressed)
				downT->start();
			else
				downT->stop();
...
int main() {
	...
	button_s = Sound::create("Sounds/button.wav");
	damaged_s = Sound::create("Sounds/damaged.wav");
	eat_s = Sound::create("Sounds/eat.wav");

	start_button = Object::create("Images/start.png", home, 565, 100);
	...
	restart_button1 = Object::create("Images/restart.png", gameover, 565, 50);
	restart_button2 = Object::create("Images/restart.png", gameclear, 565, 50);
